...
class ConfocalLogic(LogicBase):
    ...
    _history_status = StatusVar(name='history_status', default=list())
    ...
    def on_activate(self):
        ...
        for i in reversed(range(1, self.max_history_length)):
            try:
                ...
                new_history_item.deserialize(self._history_status[i])
                self.history.append(new_history_item)
            except IndexError:
                pass
            except OldConfigFileError:
                self.log.warning('Old style config file detected. History {0} ignored.'.format(i))
            except:
                self.log.warning('Restoring history {0} failed.'.format(i))
        try:
            new_state = ConfocalHistoryEntry(self)
            new_state.deserialize(self._history_status[0])
    ...
    def on_deactivate(self):
        ...
        for state in reversed(self.history):
            self._history_status.insert(histindex, state.serialize())
            if len(self._history_status) > self.max_history_length:
                del self._history_status[-1]
            histindex += 1
        return
