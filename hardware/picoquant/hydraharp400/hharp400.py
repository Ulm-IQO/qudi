...
#
# The wrappered commands are based on the hHLib Version 3.0. For further
...
class HydraHarp400(Base, SlowCounterInterface, FastCounterInterface):
    """ Hardware class to control the HydraHarp 300 from PicoQuant.
    ...
    fastcounter_hydraharp400:
        module.Class: 'picoquant.hydraharp400.hharp400.HydraHarp400'
        deviceID: 0 # a device index from 0 to 7.
        mode: 0 # 0: histogram mode, 2: T2 mode, 3: T3 mode, 8:CONT Mode
    ...
    minimal_binwidth = ConfigOption('minimal_binwidth', 1.0e-13, missing='warn')
    ...
    counts = [(ctypes.c_uint * MAXHISTLEN)() for i in range(0, HHMAXINPCHAN)]
    flags = ctypes.c_int()


    sigReadoutHydraharp = QtCore.Signal()
    ...
    def __init__(self, config, **kwargs):
        ...
        # Load the hydraharp library file hhlib64.dll from the folder
        # <Windows>/System32/
        self._dll = ctypes.cdll.LoadLibrary('hhlib64')

        # Just some default values:
        self._bin_width_ns = 3000
        self._record_length_s = 100 * 1e9

        # this variable has to be added because there is no difference
        # in the hydraharp it can be on "stopped" or "halt"
    ...
    def on_activate(self):
        ...
        _dll.HH_GetLibraryVersion(self.libVersion)
        if self.libVersion.value.decode("utf-8") != self._lib_version:
            self.log.warn("Warning: The application was built for version %s" % LIB_VERSION)

        self.open_connection()
        self.initialize(self._mode, self._refsource)
        self.calibrate()
        ...
        self.sigReadoutHydraharp.connect(self.get_fresh_data_loop,
    ...
    def on_deactivate(self):
        ...
        self.sigReadoutHydraharp.disconnect()
    ...
    def get_constraints(self):
        ...
        constraints['hardware_binwidth_list'] = list(self.minimal_binwidth * (2 ** np.array(
                                                     np.linspace(0,24,25)))) #TODO get from hardware

        constraints['max_bins'] = 65536 # taken from hhdefine.h
    ...
    def start_measure(self):
        ...
        self.meas_run = True
        # start the device:
        status = self.start(int(self._record_length_s))
        return status

    def stop_measure(self):
        """ Stop the fast counter. """
        self.stopped_or_halt = "stopped"
        status = self.tryfunc(_dll.HH_StopMeas(self._deviceID), "StopMeas")
    ...
    def continue_measure(self):
        ...
        """
        status = self.start(self._record_length_s)
        return status
    ...
    def get_binwidth(self):
        ...
        self.tryfunc(_dll.HH_GetResolution(ct.c_int(dev[0]), byref(resolution), "GetResolution"), "GetResolution")
    ...
    def get_data_trace(self):
        ...
        if self.is_gated():
            ...
            self.tryfunc(_dll.HH_GetHistogram(self._deviceID, byref(self.counts[0]), 0, 1), "GetHistogram")

        self.tryfunc(_dll.HH_GetFlags(self._deviceID, byref(self.flags)), "GetFlags")
    ...
    def _create_errorcode(self):
        ...
        filename = os.path.join(maindir, 'hardware', 'PicoQuant', 'HydraHarp400', 'errorcodes.h')
        try:
            with open(filename) as f:
                content = f.readlines()
        except:
            self.log.error('No file "errorcodes.h" could be found in the '
                           'HydraHarp hardware directory!')
    ...
    def _set_constants(self):
        ...
        # in s:
        self.ACQTMIN = 1e-3
        self.ACQTMAX = 100 * 60 * 60 * 1000 * 1e-3
        self.TIMEOUT = 80e-3  # the maximal device timeout for a readout request
    ...
    def start(self, acq_time):
        """ Start acquisition for 'acq_time' s.

        @param int acq_time: acquisition time in seconds. The value must be
        ...
        if not(self.ACQTMIN <= acq_time <= self.ACQTMAX):
            self.log.error('HydraHarp: No measurement could be started.\n'
                           ...
                           's, but a value of {2} has been passed.'
                           ''.format(self.ACQTMIN, self.ACQTMAX, acq_time))
        else:
            return self.check(self._dll.HH_StartMeas(self._deviceID, int(acq_time*1e3)))
    ...
    def set_bitshift(self, bitshift):
        ...
        self.tryfunc(self._dll.HH_GetResolution(self._deviceID, ctypes.byref(self.resolution)), "GetResolution")
        return self.resolution.value * 1e-12

    def tryfunc(self, retcode, funcName, measRunning=False):
        if retcode < 0:
            self._dll.HH_GetErrorString(self.errorString, ctypes.c_int(retcode))
