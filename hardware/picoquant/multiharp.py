...
class Multiharp150(Base, FastCounterInterface):
    """
    The MultiHarp-MHLibv20 software needs to be installed. (
    "https://www.picoquant.com/products/category/tcspc-and-time-tagging-modules/multiharp-150-high-throughput
    -multichannel-event-timer-tcspc-unit?pk_campaign=Header_Photon_Counting_MultiHarp150#custom1")

    Sample config:

    multiharp:
        module.Class: 'picoquant.multiharp.Multiharp150'
        deviceID: 0
        syncEdgeTrigger: 500 # in mV
        inputEdgeTrigger: -50 # in mV
    ...
    inputChannelOffset = ConfigOption('inputChannelOffset', 0,
                                      missing='warn')  # you can change this (in ps, like a cable delay)
    syncEdgeTrg = ConfigOption('syncEdgeTrigger', default=500, missing='warn')  # in mV
    inputEdgeTrg = ConfigOption('inputEdgeTrigger', default=-50, missing='warn')  # in mV
    LIB_VERSION = "2.0"
    MAXDEVNUM = 8
    MODE_HIST = 0
    MAXLENCODE = 6
    ...
    def configure(self, bin_width_s, record_length_s, number_of_gates=0):
        ...
        @return tuple(binwidth_s, record_length_s, number_of_gates):
                    ...
                    ...
        """
        if record_length_s >= 5.24288e-6:
            self.log.info('Record Length too long to sample set to 5.2e-6s')
            record_length_s = 5.2e-6
        # self.binning = int(bin_width_s / 80e-12) - 1
        self.binning = int(np.log2(bin_width_s / 80e-12))
        length_array = np.array([1024 * 2 ** ii for ii in range(7)]) * bin_width_s
        if record_length_s in length_array:
            lencode = np.where(length_array == record_length_s)[0][0]
        else:
            get_length = np.ceil(length_array - record_length_s)
            record_length_s = length_array[np.where(get_length > 0)[0][0]]
            lencode = np.int(np.where(get_length)[0][0])
        self._lencode = lencode
        self.error_code(self._dll.MH_SetHistoLen(ct.c_int(self._device_num), ct.c_int(lencode), ct.byref(self.histLen)),
                        "SetHistoLen")
        ...
        return float(bin_width_s), float(record_length_s), float(number_of_gates)
    ...
    def get_data_trace(self):
        ...
        """
        self.data = np.zeros([self.MAXINPCHAN, 1024 * 2 ** self._lencode], dtype=np.int32)
        # self.error_code(self._dll.MH_ClearHistMem(ct.c_int(self._device_num)), "ClearHistMem")
