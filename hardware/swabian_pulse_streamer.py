...
class PulseStreamer(Base):
    ...
    def __init__(self, **kwargs):
        self.pulsestreamer_ip = '192.168.202.200' # We should get this from the config, figure this out later
        super().__init__(**kwargs)

    def on_activate(self):
        print('Pulse streamer got activated.')
        self.pulser = ps.PulseStreamer(self.pulsestreamer_ip)

    def on_deactivate(self):
        print('Pulse streamer got deactivated.')
        self.pulser.reset() #resets pulse streamer
        del self.pulser #get rid of the object

    def run_sequence(self,sequence_dict,n_runs=-1): #change here. Do not pass sequence but dict with patterns
        """
