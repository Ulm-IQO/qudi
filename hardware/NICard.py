...
class NICard(Base,SlowCounterInterface,ConfocalScannerInterface,ODMRCounterInterface):
    ...
    def __init__(self, manager, name, config, **kwargs):
        ...
        if 'odmr_trigger_channel' in config.keys():
            ...
            self.logMsg('No odmr_trigger_channel configured, using {} instead.'.format(self._odmr_trigger_channel), msgType='warning')
...
################################## Counter ###################################
    ...
    def set_up_clock(self, clock_frequency = None, clock_channel = None, scanner=False, idle = False):
        ...
        # create handle for task, this task will generate pulse signal for photon counting
    ...
    def set_up_counter(self, counter_channel = None, photon_source = None, clock_channel = None):
        ...
        self._counter_daq_task = daq.TaskHandle()
        ...
        #    you choose)
        daq.DAQmxCreateCISemiPeriodChan(self._counter_daq_task, # The task to which to add the channels
        ...
        daq.DAQmxSetCISemiPeriodTerm(self._counter_daq_task, 
                                     self._counter_channel, 
                                     my_clock_channel+'InternalOutput')
                                     
        # define the source of ticks for the counter as self._photon_source
        daq.DAQmxSetCICtrTimebaseSrc( self._counter_daq_task, 
                                     self._counter_channel, 
                                     self._photon_source )          
        
        # set timing to continuous, i.e. set only the number of samples to 
        # acquire or generate without specifying timing
        daq.DAQmxCfgImplicitTiming( self._counter_daq_task, #define task
                                    daq.DAQmx_Val_ContSamps, #continuous running
                                    1000) #buffer length
    ...
    def get_counter(self, samples=None):
        ...
        daq.DAQmxReadCounterU32(self._counter_daq_task, #read from this task
                                ...
                                count_data, # write into this array 
...
############################ Confocal Scanner ################################
    ...
    def _start_analoque_output(self):
        ...
        # the type definition for the task
        self._scanner_ao_task = daq.TaskHandle()
        
        # actually create the analoque output task
        ...
        daq.DAQmxCreateAOVoltageChan(self._scanner_ao_task, # add to this task
                                     self._scanner_ao_channels, #use (all) sanncer ao_channels for the output
                                     '',  #assing a name
    ...
    def set_up_scanner(self, counter_channel = None, photon_source = None, clock_channel = None, scanner_ao_channels = None):
        ...
        # set task timing to on demand for the analoque output, i.e. when demanded by software
        daq.DAQmxSetSampTimingType(self._scanner_ao_task, daq.DAQmx_Val_OnDemand)
        
        # create handle for task, this task will do the photon counting for the scanner
        ...
        #    transitions of a digital signal and adds the channel to the task 
        #    you choose)
        daq.DAQmxCreateCISemiPeriodChan(self._scanner_counter_daq_task, # The task to which to add the channels
                                        self._scanner_counter_channel,  # use this counter channel
                                        '', #name to assing to it
                                        0, #expected minimum value
                                        self._max_counts/self._scanner_clock_frequency, #expected maximum count value
                                        daq.DAQmx_Val_Ticks, #units of width measurement, here photon ticks
                                        '')
    ...
    def _write_scanner_ao(self, voltages, length=1 ,start=False):
        ...
        daq.DAQmxWriteAnalogF64(self._scanner_ao_task, #write to this task
                                length,  # length of the command (points)
                                start, # start task immediately (True), or wait for software start (False)
                                self._RWTimeout, # maximal timeout for the write process
                                daq.DAQmx_Val_GroupByChannel, # each pixel is grouped by Channel number
    ...
    def _scanner_position_to_volt(self, positions = None):
        ...
        * (positions[3]-self._position_range[3][0])\
        ...
        if volts.min() < self._voltage_range[0] or volts.max() > self._voltage_range[1]:
            self.logMsg('Voltages exceed the limit, the positions have to be adjusted to stay in the given range.', \
            msgType='error')
            return np.array([-1.,-1.,-1.,-1.])
    ...
    def set_up_line(self, length=100):
        ...
        if length < np.inf:
            ...
            daq.DAQmxCfgSampClkTiming(self._scanner_ao_task, # add to this task
                                      ...
                                      daq.DAQmx_Val_Falling, #  generate sample on falling edge
...
#        if np.shape(voltages)[1] != self._line_length:
        ...
        daq.DAQmxReadCounterU32(self._scanner_counter_daq_task, #read from this task
                                2*self._line_length, #read number of double the number of samples
                                self._RWTimeout, # maximal timeout for the read process
        ...
        daq.DAQmxSetSampTimingType(self._scanner_ao_task, # define task
                                   daq.DAQmx_Val_OnDemand) # sampling directly when a voltage is written
        
        # create a new array for the final data (this time of the length number of samples)
        self._real_data = np.empty((self._line_length,), dtype=np.uint32)
        
        # add upp adjoint pixels to also get the counts from the low time of the clock
...
################################# ODMR ######################################
    ...
    def set_up_odmr(self, counter_channel = None, photon_source = None, clock_channel = None, odmr_trigger_channel = None):
        ...
        daq.DAQmxCreateCISemiPeriodChan(self._scanner_counter_daq_task, # The task to which to add the channels
                                        ...
                                        'Counting Task',  #name to assing to it
                                        0,  #expected minimum count value
                                        self._max_counts/2./self._scanner_clock_frequency,    #expected maximum count value
                                        daq.DAQmx_Val_Ticks, #units of width measurement, here photon ticks
                                        '')   
