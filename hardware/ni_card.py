...
class NICard(Base, SlowCounterInterface, ConfocalScannerInterface, ODMRCounterInterface):
    ...
    def set_up_fixed_counter(self, samples,
        ...
        try:
            ...
            # Initialize a Task
            task = daq.TaskHandle()
            daq.DAQmxCreateTask('FixedCounter', daq.byref(task))
    ...
    def start_fixed_counter(self):
        ...
        if self._scanner_counter_daq_tasks is None:
            self.log.error(
                'Cannot start Fixed Counter Task since it is notconfigured!\n'
                'Run the set_up_fixed_counter routine.')
            return -1
        elif len(self._scanner_counter_daq_tasks) > 1:
            self.log.error('To many ({}) Scanner Counter Tasks defined. Close all scanner '
                           'counters. \n Then resetup the fixed counter. '.format(
        ...
        try:
            ...
            self.log.error('Error while starting up fixed counter clock')
            return -1
