...
class NICard(Base, SlowCounterInterface, ConfocalScannerInterface, ODMRCounterInterface):
    ...
    def get_status(self):
        ...
        -1 = error state
        """
        if len(self._gated_counter_daq_tasks) < 1:
            return 0
        else:
            ...
            for i, task in enumerate(self._gated_counter_daq_tasks):
    ...
    def set_up_gated_counter(self, buffer_length, read_available_samples=False):
        ...
        @param bool read_available_samples: if False, NiDaq waits for the
                                            ...
                                            is full
        """
        if len(self._gated_counter_daq_tasks) > 0:
            self.log.error('Another gated counter is already running, close this one first.')
            return -1
        ...
        try:
            for i, ch in enumerate(my_counter_channels):
                ...
                # Initialize a Task
                task = daq.TaskHandle()
                daq.DAQmxCreateTask('GatedCounter{0}'.format(i), daq.byref(task))
                ...
                daq.DAQmxCreateCIPulseWidthChan(
                    ...
                    task,
                    # use this counter
                    my_counter_channels[i],
                    # name you assign to it
                    'Gated Counting Task{0}'.format(i),
                    # expected minimum value
                    0,
                ...
                if read_available_samples:
                    daq.DAQmxSetReadReadAllAvailSamp(task, True)
                ...
                self._gated_counter_daq_tasks.append(task)
        except:
    ...
    def start_gated_counter(self):
        ...
        """
        if len(self._gated_counter_daq_tasks) < 1:
            self.log.error('Cannot start gated counter task since no task is configured!\n'
                           'Run the set_up_gated_counter method.')
            return -1

        try:
            for i, task in enumerate(self._gated_counter_daq_tasks):
                # Actually start the preconfigured gated counter task
                daq.DAQmxStartTask(task)
        except:
            self.log.exception('Error while starting gated counter tasks.')
            try:
                self.close_gated_counter()
            except:
                self.log.exception('Could not close gated counter tasks after error while starting '
                                   'them.')
            return -1
        return 0
    ...
    def get_gated_counts(self, samples=None, timeout=None, read_available_samples=False):
        ...
        # Count data will be written here
        _gated_count_data = np.zeros([len(self._gated_counter_daq_tasks), samples], dtype=np.uint32)

        if read_available_samples:
            # If the task acquires a finite number of samples and you set this parameter to -1,
            # the function waits for the task to acquire all requested samples,
            # then reads those samples.
            num_samples = -1
        else:
        ...
        try:
            ...
            # Chops the array or read sample to the length that it exactly returns acquired data
            # and not more
            if read_available_samples:
            ...
            return np.zeros([len(self._gated_counter_daq_tasks), samples], dtype=np.uint32) - 1
    ...
    def stop_gated_counter(self):
        ...
        """
        if len(self._gated_counter_daq_tasks) < 1:
            self.log.error('Cannot stop gated counter tasks since it is not running!\n'
    ...
    def close_gated_counter(self):
        ...
        try:
            ...
            for i, task in enumerate(self._gated_counter_daq_tasks):
                daq.DAQmxClearTask(task)
            self._gated_counter_daq_tasks = list()
        except:
