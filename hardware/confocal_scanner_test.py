...
class ConfocalScannerTest(Base, ConfocalScannerInterface):

    """ Dummy confocal scanner.
        Produces a picture with several gaussian spots.
    """
    _modclass = 'ConfocalScannerTest'
    _modtype = 'hardware'
    ...
    def on_activate(self):
        ...
        self._points[:, 0] = 2e9
        # x_zero
        self._points[:, 1] = 20e-6 #np.random.uniform(self._position_range[0][0], self._position_range[0][1], self._num_points)
        # y_zero
        self._points[:, 2] = 20e-6 #np.random.uniform(self._position_range[1][0], self._position_range[1][1], self._num_points)
        # sigma_x
        self._points[:, 3] = 0.5e-6 #np.random.normal(0.7e-6, 0.1e-6, self._num_points)
        # sigma_y
        self._points[:, 4] = 0.5e-6 #np.random.normal(0.7e-6, 0.1e-6, self._num_points)
        # theta
        self._points[:, 5] = 0
        # offset
        self._points[:, 6] = 0
...
#       gaussian_function(self,x_data=None,amplitude=None, x_zero=None, sigma=None, offset=None):
        ...
        self._points_z[:, 0] = 1
        # x_zero
        self._points_z[:, 1] = np.random.uniform(45e-6, 55e-6, self._num_points)
        # sigma
        self._points_z[:, 2] = np.random.normal(0.5e-6, 0.1e-6, self._num_points)
        # offset
        self._points_z[:, 3] = 0
