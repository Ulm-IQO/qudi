...
class Qutau(Base, FastCounterInterface):
    ...
    def get_last_timestamps(self, reset=True):
        ...
        if self._number_of_gates:
            timestamps = np.zeros(int(self._buffer_size),
    ...
    def on_activate(self):
        ...
        self._dll.TDC_createLftFunction.restype = ctypes.POINTER(TDC_LftFunction)
        self._dll.TDC_getLftHistogram.argtypes = [ctypes.c_int32, ctypes.c_int32, ctypes.POINTER(TDC_LftFunction),
    ...
    def configure(self, bin_width_s, record_length_s, number_of_gates=0):
        ...
        self._buffer_size = int(np.rint(record_length_s / bin_width_s))  # calculate the lenght of the histogram array
        # in multiples of bin
        self._bin_size = int(bin_width_s / 81e-12)  # calculate the binning size in multiples of smallest bin size
        # self.enable_start_stop(1)  # enable the start single-stop measurement
        self.enable_LFT(1)
        # self.set_histogram_params(self._bin_size, self._buffer_size)  # set the bin size and buffer size for start single-stop
        self.set_LFTParams(self._bin_size, self._buffer_size)  # set the bin size and buffer size for start multi-stop
    ...
    def start_measure(self):
        ...
        # self.set_histogram_params(self._bin_size, self._buffer_size)  # set the histo params as above in config
        # self.freeze_buffers(0)  # release buffers to start histogram exposure
        self.enable_LFT(1)
        self.set_LFTParams(self._bin_size, self._buffer_size)
        self._lft_function = self.create_LFTFunction()
        self.freeze_buffers(0)
        # self.release_LFTFunction(self._lft_function)
    ...
    def get_binwidth(self):
        ...
        """
        return self._bin_size  # todo: maybe not correct
    ...
    def get_data_trace(self):
        ...
        """
        # for start single-stop
        # data, count, tooSmall, tooLarge, starts, stops, expTime = self.get_histogram(self._trigger_channel,
        #                                                                              self._count_channel, 0)
        # for start multi-stop
        capacity, size, bin_width, data = self.analyse_LFTFunction(self._lft_function)
    ...
    def enable_LFT(self, enable):
        if enable:
            ena = 1
        else:
            ena = 0
        ans = self._dll.TDC_enableLft(ena)
        if ans != 0:
            print("Error in TDC_enableLft: " + self.err_dict[ans])
        return ans

    def set_LFTParams(self, binWidth, binCount):
        self._LFTBufferSize = binCount
        ans = self._dll.TDC_setLftParams(binWidth, binCount)
        if ans != 0:
            print("Error in TDC_setLftParams: " + self.err_dict[ans])
        return ans

    def get_LFTParams(self):
        binWidth = ctypes.c_int32()
        binCount = ctypes.c_int32()
        ans = self._dll.TDC_getLftParams(ctypes.byref(binWidth), ctypes.byref(binCount))
        if ans != 0:
            print("Error in TDC_getLftParams: " + self.err_dict[ans])
        return binWidth.value, binCount.value

    def set_LFTStartInput(self, startChannel):
        ans = self._dll.TDC_setLftStartInput(startChannel)
        if ans != 0:
            print("Error in TDC_setLftStartInput: " + self.err_dict[ans])
        return ans

    def get_LFTStartInput(self):
        startChannel = ctypes.c_int32()
        ans = self._dll.TDC_getLftStartInput(ctypes.byref(startChannel))
        if ans != 0:
            print("Error in TDC_getLFTStartInput: " + self.err_dict[ans])
        return startChannel.value

    def reset_LFTHistograms(self):
        ans = self._dll.TDC_resetLftHistograms()
        if ans != 0:
            print("Error in TDC_resetLftHistrograms: " + self.err_dict[ans])
        return ans

    def create_LFTFunction(self):
        LFTfunction = self._dll.TDC_createLftFunction()
        return LFTfunction

    def release_LFTFunction(self, LFTfunction):
        self._dll.TDC_releaseLftFunction(LFTfunction)
        return 0

    def add_LFTHistogram(self, stopchannel, enable):
        if enable:
            ena = 1
        else:
            ena = 0

        ans = self._dll.TDC_addLftHistogram(stopchannel, ena)
        if ans != 0:
            print("Error in TDC_addLftHistogram: " + self.err_dict[ans])
        return ans

    def analyse_LFTFunction(self, lft):
        capacity = ctypes.c_int32()
        size = ctypes.c_int32()
        bin_width = ctypes.c_int32()
        values = np.zeros(self._LFTBufferSize, dtype=np.double)

        self._dll.TDC_analyseLftFunction(lft, ctypes.byref(capacity), ctypes.byref(size), ctypes.byref(bin_width),
                                         values.ctypes.data_as(ctypes.POINTER(ctypes.c_double)),
                                         self._LFTBufferSize)
        return capacity.value, size.value, bin_width.value, values

    def get_LFTHistogram(self, channel, reset, lft):
        too_big = ctypes.c_int32()
        start_evt = ctypes.c_int32()
        stop_evt = ctypes.c_int32()
        exp_time = ctypes.c_int64()
        if reset:
            reset_value = 1
        else:
            reset_value = 0

        ans = self._dll.TDC_getLftHistogram(channel, reset_value, lft, ctypes.byref(too_big), ctypes.byref(start_evt),
                                            ctypes.byref(stop_evt), ctypes.byref(exp_time))
        if ans != 0:
            print("Error in TDC_getLFTHistogram: " + self.err_dict[ans])
        return too_big.value, start_evt.value, stop_evt.value, exp_time.value

    def calc_LFTModelFCT(self, lfttype, params, lftfunction):
        ...
        for i in range(len(params)):
            if i < self.LFT_PARAM_SIZE:
                ...
                break
        ans = self._dll.TDC_calcLftModelFct(lfttype, c_params.ctypes.data_as(ctypes.POINTER(ctypes.c_double)),
                                            lftfunction)
        if ans != 0:
            print("Error in TDC_calcLftModelFct: " + self.err_dict[ans])
        return ans

    def generate_LFTDemo(self, lfttype, params, noiseLv):
        c_params = np.zeros(self.LFT_PARAM_SIZE, dtype=np.double)
        for i in range(len(params)):
            if (i < self.LFT_PARAM_SIZE):
                c_params[i] = params[i]
                ...
                break
        ans = self._dll.TDC_generateLftDemo(lfttype, c_params.ctypes.data_as(ctypes.POINTER(ctypes.c_double)),
                                            noiseLv)
        if ans != 0:
            print("Error in TDC_generateLftDemo: " + self.err_dict[ans])
        return ans

    def fit_LFTHistogram(self, lft, lfttype, startParams):
        c_params = np.zeros(self.LFT_PARAM_SIZE, dtype=np.double)
        ...
        ans = self._dll.TDC_fitLftHistogram(lft, lfttype,
    ...
    def test_LFT(self):
        ...
        # self.enable_LFT(True)
        self.set_LFTStartInput(0)
        self.reset_LFTHistograms()
        # self.set_LFTParams(1, 1000)
        lft_function = self.create_LFTFunction()
        capacity, size, bin_width, values = self.analyse_LFTFunction(lft_function)
        print(values)
        # self.get_LFTHistogram(1, False, lft_function)

        # self.get_LFTHistogram(1, 0, lft_function)
        # print(self.analyse_LFTFunction(lft_function))
