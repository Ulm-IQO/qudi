...
class InStreamDummy(Base, DataInStreamInterface):
    ...
    instream_dummy:
        ...
        analog_voltage_ranges:  # optional, must have as many entries as analog_channels or just one
            - [-1, 1]
            - [-5, 5]
            - [-10, 10]
        # analog_voltage_ranges: [-10, 10]  # optional (-10V..10V by default)
    """
    # config options
    ...
    _digital_event_rates = ConfigOption(name='digital_event_rates',
                                        default=100000,
                                        missing='nothing')
    _analog_voltage_ranges = ConfigOption(name='analog_voltage_ranges',
                                          default=(-10, 10),
                                          missing='nothing')
    ...
    def on_activate(self):
        ...
        self._digital_channels = tuple(str(chnl) for chnl in self._digital_channels)
        self._analog_channels = tuple(str(chnl) for chnl in self._analog_channels)

        try:
        if len(self._digital_channels) !=
