...
class AWG70K(Base, PulserInterface, MicrowaveInterface):
    ...
    def _compile_sine_waveform(self, name, frequency, amplitude, cycles):
        # self.log.warning('whats wrong with compile sine waveform')
        # self.log.warning(name,frequency, amplitude, cycles)
        frequency = float(frequency)
    ...
    def _create_mw_sweep_sequence(self, frequencies, power):
        ...
        # because triggering works different for me need to have the first sequence twice
        samples_per_period = sample_rate / frequencies[0]
        cycles = max(100, int(np.ceil(2 * self.__min_waveform_length / samples_per_period)))
        samples = np.round(cycles * samples_per_period)
        while samples % 2 != 0:
            cycles += 1
            samples = np.round(cycles * samples_per_period)
        name = '_mw_sweep_{0:d}_ch1'.format(0)
        self._compile_sine_waveform(name, frequencies[0], amp, cycles)
        self.sequence_set_waveform(sequence_name='_mw_sweep',
                                   waveform_name=name,
                                   step=1,
                                   track=1)
        self.sequence_set_repetitions(sequence_name='_mw_sweep', step=1, repeat=-1)
        self.sequence_set_event_jump(sequence_name='_mw_sweep',
                                     step=1,
                                     trigger=self._microwave_trigger,
                                     jumpto=-1)

        for ii, freq in enumerate(frequencies, 2):
            ...
            while samples % 2 != 0:
                cycles += 1
                samples = np.round(cycles * samples_per_period)
            # will the number of samples be even?
