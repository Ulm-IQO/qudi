...
def plot_b_hist(mes_list, savepath=None, filter_str_success='combine'):
    ...
    b_min_1 = np.min(b_arr[:,0])
    b_max_1 = np.max(b_arr[:,0])
    b_min_2 = np.min(b_arr[:,1])
    b_max_2 = np.max(b_arr[:,1])
...
if __name__ == '__main__':
    ...
    - folder E: MFL runs are stored with wrong result. Usually those have .fail indicator in 
      the folder name
    - folder C: assignment what's wrong needs to be done manually before. To this end, in folder C, there
      is a measurement of what is believed to be the correct result
    """
    ...
    for path in paths:
        ...
        #"""
        # b
        # todo: seems broken for single parameter estimate
