...
class PoI(object):
    ...
    def __init__(self, point=None, name=None, key=None):
        ...
        if point is not None:
            if len(point) != 3:
                logger.error('Given position does not contain 3 dimensions.')
    ...
    def set_name(self, name=None):
        ...
        if self._name is 'crosshair' or self._name is 'sample':
            #            logger.error('You can not change the name of the crosshair.')
            return -1
        if name is not None:
            self._name = name
            return 0
...
#        print("Confocal Logic is", self._confocal_logic)
    ...
    def delete_poi(self, poikey=None):
        ...
        if poikey is not None and poikey in self.track_point_list.keys():
            if poikey is 'crosshair' or poikey is 'sample':
                logger.warning('You cannot delete the crosshair or sample.')
                return -1
            del self.track_point_list[poikey]
            self.signal_poi_updated.emit()
            return 0
        else:
            logger.error('X. The given POI ({}) does not exist.'.format(
                poikey))
            return -1
    ...
    def optimise_poi(self, poikey=None):
        ...
        if poikey is not None and poikey in self.track_point_list.keys():
            ...
            logger.error('Z. The given POI ({}) does not exist.'.format(
                poikey))
            return -1
    ...
    def go_to_poi(self, poikey=None):
        ...
        if poikey is not None and poikey in self.track_point_list.keys():
            ...
            logger.error('F. The given POI ({}) does not exist.'.format(
                poikey))
            return -1
    ...
    def get_poi_position(self, poikey=None):
        ...
        if poikey is not None and poikey in self.track_point_list.keys():
            ...
            logger.error('G. The given POI ({}) does not exist.'.format(
                poikey))
            return [-1., -1., -1.]
    ...
    def set_new_position(self, poikey=None, point=None):
        ...
        if poikey is not None and poikey in self.track_point_list.keys():
            if len(point) != 3:
                logger.error('Length of set poi is not 3.')
                return -1
        ...
        logger.error('J. The given POI ({}) does not exist.'.format(poikey))
        return -1
    ...
    def rename_poi(self, poikey=None, name=None, emit_change=True):
        ...
        if poikey is not None and name is not None and poikey in self.track_point_list.keys():
            ...
            logger.error('AAAThe given POI ({}) does not exist.'.format(
                poikey))
            return -1
    ...
    def delete_last_point(self, poikey=None):
        ...
        if poikey is not None and poikey in self.track_point_list.keys():
            ...
            logger.error('C. The given POI ({}) does not exist.'.format(
                poikey))
            return -1
    ...
    def set_current_poi(self, poikey=None):
        ...
        if poikey is not None and poikey in self.track_point_list.keys():
            ...
            logger.error('B. The given POI ({}) does not exist.'.format(
                poikey))
            return -1
    ...
    def start_periodic_refocus(self, duration=None, poikey=None):
        ...
        if duration is not None:
            ...
            logger.warning('No timer duration given, using {} s.'.format(
                self.timer_duration))

        if poikey is not None and poikey in self.track_point_list.keys():
            self._current_poi_key = poikey

        logger.info('Periodic refocus on {}.'.format(self._current_poi_key))
    ...
    def _refocus_done(self, caller_tag, optimal_pos):
        ...
        # If the refocus was initiated here by poimanager, then update POI and sample
            ...
            if self._current_poi_key is not None and self._current_poi_key in self.track_point_list.keys():
                ...
                logger.error('W. The given POI ({}) does not exist.'.format(
                    self._current_poi_key))
                return -1

        else:
            logger.error('Unknown caller_tag for the optimizer. POI Manager '
                    'does not know what to do with optimized position, and '
                    'has done nothing.')
