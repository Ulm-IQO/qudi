...
########################################################################################################################
        ...
        def estimate_sine(self,x_axis=None,data=None):
            ...
            for var in parameters:
                if not isinstance(var,(frozenset, list, set, tuple, np.ndarray)):
                    ...
                    self.logMsg('Given parameter is no one dimensional array.', msgType='error')
                    error=-1
        ...
        def fcn2min(self,params, x, data):
            ...
            model = v['amplitude'] * np.sin(x * 1/v['omega']*np.pi*2 + v['shift']) * np.exp(-x*v['decay']) + v['offset']
            return model - data
        ...
        def make_sine_fit(self,axis=None,data=None,add_parameters=None):
            """ This method performes a sine fit on the provided data.
                    ...
                    @return result: All parameters provided about the fitting, like: success,
                                    initial fitting values, best fitting values, data with best
                                    fit with given axis,...
            ...
            amplitude, offset, shift, omega, decay = self.estimate_sine(axis,data)
