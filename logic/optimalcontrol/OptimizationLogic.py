...
class OptimizationLogic(LogicBase, OptimizationBasic):
    ...
    def connect_fom(self):
        """ Connect fom signals """
        # Create fom object and handle exit object
        self.handle_exit_obj = HandleExitLogic()
    ...
    def disconnect_fom(self):
        """Disconnect the signals for the figure of merit object """
        # Creation of the figure of merit class to be use in the optimization dictionary to allow the transmissions
        # between the figure of merit logic and optimization logic
        self.send_controls_signal.disconnect()
        # Very important here: Use the Direct Connection instead the default one, otherwise the signal will never reach
        # the slot
        self.fom_logic().send_fom_signal.disconnect()
        # Handle exit signals
        self.handle_exit_obj.is_optimization_running_fom_signal.disconnect()
        self.handle_exit_obj.is_optimization_running_controls_signal.disconnect()
        self.wait_fom_signal.disconnect()
        self.is_running_signal.disconnect()
        # Delete the figure of merit object
        del self.fom_obj
        del self.handle_exit_obj
    ...
    def start_optimization(self, opti_comm_dict):
        ...
        self.is_fom_computed = False
        self.fom_logic.is_fom_computed = False
        if self.handle_exit_obj.is_user_running:
        ...
        self.log.info("The optimization is initialized successfully.")
        try:
        ...
        # Disconnect all the signals and delete the fom object
        self.disconnect_fom()
        return
