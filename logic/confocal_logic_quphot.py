...
#            time.sleep(0.01)
    ...
    def initialize_image(self):
        ...
        a1, a2 = self.image_z_range[0], self.image_z_range[1]
        ...
        if self._zscan:
            if self.depth_img_is_xz:
                ...
                self._A = np.linspace(a1, a2, max(self.z_resolution, 2))
            else:
        ...
        self._return_AL = np.linspace(self._AL[-1], self._AL[0], 50)
        ...
        if self._zscan:
            ...
            if self.depth_img_is_xz:
                ...
                self.depth_image[:, :, 0] = self._current_x * np.ones(
                    (len(self._image_vert_axis), len(self._A)))

                self.depth_image[:, :, 1] = self._current_y * np.ones(
                    (len(self._image_vert_axis), len(self._A)))

                z_value_matrix = np.full((len(self._A), len(self._image_vert_axis)), self._Z)
                self.depth_image[:, :, 2] = z_value_matrix.transpose()

                self.depth_image[:, :, 3] = np.full(
                    (len(self._image_vert_axis), len(self._A)), self._AL)
            ...
            # depth scan is yz plane instead of xz plane
                ...
                self.depth_image[:, :, 3] = self._current_a * np.ones(
                    (len(self._image_vert_axis), len(self._Y)))
        ...
        # xy scan is in xy plane
            ...
            self.xy_image[:, :, 3] = self._current_a * np.ones(
                (len(self._image_vert_axis), len(self._X)))
    ...
    def _scan_line(self):
        ...
        try:
            if self._scan_counter == 0:
                ...
                lsa = np.linspace(self._current_a, image[self._scan_counter, 0, 3], 50)
                if n_ch <= 3:
            ...
            lsa = image[self._scan_counter, :, 3]
            if n_ch <= 3:
            ...
            if self.depth_img_is_xz and not self._zscan:
                if n_ch <= 3:
                    ...
                    return_line = np.vstack([
                            ...
                            image[self._scan_counter, 0, 1] * np.ones(self._return_XL.shape),
                            image[self._scan_counter, 0, 2] * np.ones(self._return_XL.shape),
                            image[self._scan_counter, 0, 3] * np.ones(self._return_XL.shape)
                        ])
            elif self._zscan:
                if n_ch <= 3:
                    return_line = np.vstack([
                    ...
                    return_line = np.vstack([
                            image[self._scan_counter, 0, 0] * np.ones(self._return_AL.shape),
                            image[self._scan_counter, 0, 1] * np.ones(self._return_AL.shape),
                            image[self._scan_counter, 0, 2] * np.ones(self._return_AL.shape),
                            self._return_AL
                        ])
            else:
                if n_ch <= 3:
            ...
            if self._zscan:
                if self.depth_img_is_xz:
                    self.depth_image[self._scan_counter, :, 4:4 + s_ch] = line_counts
                else:
                    self.depth_image[self._scan_counter, :, 4:4 + s_ch] = line_counts
                self.signal_depth_image_updated.emit()
            else:
                self.xy_image[self._scan_counter, :, 4:4 + s_ch] = line_counts
                self.signal_xy_image_updated.emit()
    ...
    def _save_xy_data(self, colorscale_range=None, percentile_range=None):
        ...
        figs = {ch: self.draw_figure(data=self.xy_image[:, :, 4 + n],
        ...
        for n, ch in enumerate(self.get_scanner_count_channels()):
            ...
            image_data['Confocal pure XY scan image data without axis.\n'
                ...
                'of entries where the Signal is in counts/s:'] = self.xy_image[:, :, 4 + n]
    ...
    def save_depth_data(self, colorscale_range=None, percentile_range=None):
        ...
        figs = {ch: self.draw_figure(data=self.depth_image[:, :, 4 + n],
                                     image_extent=image_extent,
                                     scan_axis=axes,
                                     cbar_range=colorscale_range,
                                     percentile_range=percentile_range,
        ...
        for n, ch in enumerate(self.get_scanner_count_channels()):
            ...
            image_data['Confocal pure depth scan image data without axis.\n'
                ...
                'of entries where the Signal is in counts/s:'] = self.depth_image[:, :, 4 + n]
        ...
        for n, ch in enumerate(self.get_scanner_count_channels()):
            data['count rate {0} (Hz)'.format(ch)] = self.depth_image[:, :, 4 + n].flatten()
