...
class ConfocalGui(GUIBase):
    ...
    def initMainUI(self):
        ...
        *.ui file and configures the event handling between the modules.
        ...
        self._mw.set_confocal_to_programmable.setText(
            "{:3.1f}:{:3.1f}".format(float(self._x_programmable) * 1e6, float(self._y_programmable) * 1e6))
    ...
    def set_confocal_crosshair_to_programmable(self):
        ...
        self._mw.y_current_InputWidget.setValue(self._y_programmable)
        self._mw.x_current_InputWidget.editingFinished.emit()
        self._mw.y_current_InputWidget.editingFinished.emit()

    def set_new_location_programmable(self):
        position = self._scanning_logic.get_position()
        x_pos = position[0]
        y_pos = position[1]
        self._x_programmable = x_pos
        self._y_programmable = y_pos
        self._mw.set_confocal_to_programmable.setText(
            "{:3.1f}:{:3.1f}".format(float(self._x_programmable) * 1e6, float(self._y_programmable) * 1e6))
    ...
    def update_crosshair_position_from_logic(self, tag):
        ...
        if 'roi' not in tag and 'slider' not in tag and 'key' not in tag and 'input' not in tag:
            ...
            if self._scanning_logic.depth_img_is_xz:
                self._mw.depth_ViewWidget.set_crosshair_pos((a_pos, z_pos))
            else:
    ...
    def roi_depth_bounds_check(self, pos):
        """ Check if the focus cursor is oputside the allowed range after drag
            and set its position to the limit """
        if self._scanning_logic.depth_img_is_xz:
            h_range = self._scanning_logic.z_range
        else:
    ...
    def update_from_roi_depth(self, pos):
        ...
        if self._scanning_logic.depth_img_is_xz:
            ...
            # self.update_slider_x(h_pos)
            # self.update_input_x(h_pos)
            self._scanning_logic.set_position('roidepth', a=h_pos, z=v_pos)
            self._optimizer_logic.set_position('roidepth', a=h_pos, z=-v_pos)
        else:
    ...
    def adjust_depth_window(self):
        ...
        if self._scanning_logic.depth_img_is_xz:
            self._mw.depth_ViewWidget.setLabel('bottom', '"A" position', units='m')
            xMin = self._scanning_logic.image_z_range[0]
            xMax = self._scanning_logic.image_z_range[1]
        else:
