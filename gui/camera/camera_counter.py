...
class CameraWindow(QtWidgets.QMainWindow):
    ...
    def __init__(self):
        # Get the path to the *.ui file
        this_dir = os.path.dirname(__file__)
        ui_file = os.path.join(this_dir, 'ui_camera_counter.ui')
...
class CameraCounterGUI(GUIBase):
    """ Main spectrometer camera class.
    """
    _modclass = 'CameraCounterGui'
    _modtype = 'gui'
    ...
    def on_activate(self):
        ...
        self.counts = np.zeros(self.buffer_length)
        self.times = np.ones(self.buffer_length) * time.time()

        self._logic = self.camera_logic()
        self._save_logic = self.savelogic()
        self.xy_pixels = self._logic.get_size()
        ...
        self.count_trace = pg.PlotDataItem(pen=pg.mkPen(palette.c1), symbol=None)
        self._pw.addItem(self.count_trace)
    ...
    def update_data(self):
        ...
        #TODO: Make this depend on the selected area
        count = np.average(raw_data_image) / self._logic._exposure
        ...
        self.count_trace.setData(x=self.times, y=self.counts)
        if 0 < count < 10:
            self._mw.count_value_Label.setText('{0:,.6f)'.format(count))
        else:
            self._mw.count_value_Label.setText('{0:,.0f}'.format(count))
