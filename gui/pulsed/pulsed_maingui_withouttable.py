...
class SimplePulseGeneratorTab(QtWidgets.QWidget):
    def __init__(self):
        # Get the path to the *.ui file
        this_dir = os.path.dirname(__file__)
        ui_file = os.path.join(this_dir, 'ui_simple_pulse_generator.ui')
        # Load it
        super().__init__()
        uic.loadUi(ui_file, self)
...
class PulsedMeasurementGui(GUIBase):
    ...
    def update_ensemble_list(self, ensemble_list):
        ...
        self._pg.gen_ensemble_ComboBox.addItems(ensemble_list)
        # unblock signals
