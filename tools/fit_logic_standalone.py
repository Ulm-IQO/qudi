...
#            para={}
 #           para['phase'] = {'vary': False, 'value': np.pi/2.}
  #          para['amplitude'] = {'min': 0.0}
    ...
    result=qudi_fitting.make_sine_fit(axis=x_axis,data=data_noisy,add_parameters=None)
##            result=qudi_fitting.make_powerfluorescence_fit(axis=data[:,0],data=data[:,2]/1000,add_parameters=para)
#
#            print(result.fit_report())
...
def exponentialdecay_testing():
    ...
    mod, params = qudi_fitting.make_exponentialdecay_model()
    print('Parameters of the model', mod.param_names, ' with the independet variable', mod.independent_vars)
    ...
    result = qudi_fitting.make_exponentialdecay_fit(axis=x_axis, data=data_noisy, add_parameters=None)
    data = data_noisy
...
def bareexponentialdecay_testing():
    ...
    linear_result = qudi_fitting.make_linear_fit(axis=x_axis[0:i],data= data_log,add_parameters=None)
    ...
    result = qudi_fitting.make_bareexponentialdecay_fit(axis=x_axis, data=data_noisy, add_parameters=None)
    plt.plot(x_axis, data_noisy, 'ob')
    plt.plot(x_nice, mod.eval(x=x_nice, params=params), '-g')
    print(result.fit_report())
    plt.plot(x_axis, result.init_fit, '-y', linewidth=2.0)
...
def sineexponentialdecay_testing():
    ...
    result = qudi_fitting.make_sineexponentialdecay_fit(axis=x_axis, data=data_noisy, add_parameters=None)
    plt.plot(x_axis, data_noisy, 'ob')
    plt.plot(x_nice,mod.eval(x=x_nice, params=params),'-g')
    print(result.fit_report())
    plt.plot(x_axis, result.init_fit, '-y', linewidth=2.0, )
