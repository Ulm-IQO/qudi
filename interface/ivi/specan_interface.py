...
class MarkerType(Enum):
    ...
    """
    normal = 1  # Regular marker used to make absolute measurements.
    delta = 2   # Marker used in conjunction with the reference marker to make relative measurements.
...
class SpecAnIviInterface(metaclass=InterfaceMetaclass):
    ...
    class sweep_coupling:
        ...
        def configure(self, resolution_bandwidth, video_bandwidth, sweep_time):
            ...
            :param resolution_bandwidth: either boolean, Enables or disables resolution bandwidth auto coupling. The
                                                         driver uses this value to set the Resolution Bandwidth Auto
                                                         attribute. See the attribute description for more details.
                                         or float, Specifies the measurement resolution bandwidth in Hertz. This value
                                                   is ignored when ResolutionBandwidthAuto is True. The driver uses
                                                   this value to set the Resolution Bandwidth attribute. See the
                                                   attribute description for more details.
            :param video_bandwidth: either boolean, Enables or disables video bandwidth auto coupling. The driver uses
                                    ...
                                    or float, Specifies the video bandwidth of the postdetection filter in Hertz.
                                              The driver uses this value to set the Video Bandwidth attribute. See
                                              the attribute description for more details.
            :param sweep_time: either boolean, Enables or disables sweep time auto coupling. The driver uses this value
...
class DisplayExtensionInterface(abc.ABCMeta):
    """
...
class DeltaMarkerExtensionInterface(metaclass=abc.ABCMeta):
    ...
    class markers(metaclass=abc.ABCMeta):
        ...
        def delta(self):
            """
            This function specifies whether the marker is a delta marker.

            When True, the Marker Type attribute is set to Delta. The Reference Marker Amplitude and Reference
            Marker Position attributes are set with the marker current Marker Amplitude and Marker Position attribute
            respectively.

            While the marker remains a delta marker, the values of the Marker Amplitude and Marker Position
            attributes will be relative to the Reference Marker Amplitude and Reference Marker Position attributes,
            respectively.

            When False, the marker is changed to a normal marker. The Marker Type attribute is set to Normal. The
            Marker Amplitude and Marker Position attributes are returned to a normal state and are no longer relative
            to the Reference Marker Amplitude and Reference Marker Position attributes.
            """
            pass
...
class PreselectorExtensionInterface(metaclass=abc.ABCMeta):
    """
    The IviSpecAnPreselector extension controls preselectors. Preselectors are a network of filters and
    preamplifiers that are built into one unit for reducing noise and increasing dynamic range of an analyzer.
    Preselectors are often separate instruments, but they are instruments that only work with spectrum analyzers.
    Some analyzers have internal preselectors.
    """
